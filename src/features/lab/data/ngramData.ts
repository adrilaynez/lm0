 ...
 */
export function contextDistribution(k: number, context: string): { followers: Follower[]; total: number } | null {
    const row = getCounts(k).get(context);
    if (!row) return null;
    let total = 0;
 ...
 */
export function predictAt(text: string, pos: number, k: number): { context: string; followers: Follower[]; total: number } | null {
 ...
 */
function sampleNext(k: number, context: string, temperature: number, rng: () => number): string | null {
    const d = contextDistribution(k, context);
...
export function validateAgainstBigram(): { mismatches: number; sample: string[] } {
    ...
    for (let r = 0; r < ALPHA_27.length; r++) {
        ...
        for (let c = 0; c < ALPHA_27.length; c++) {
            ...
            if (got !== want) {
                mismatches++;
                if (sample.length < 12) sample.push(`[${displayChar(ctx)}→${displayChar(next)}] got ${got}, want ${want}`);
            }
        }
    }
