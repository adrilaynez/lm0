...
function SimpleChart({
  ...
  if (allVals.length < 2)
    return (
      <p className="text-[10px] text-white/20 font-mono italic text-center py-4">
        {t("models.mlp.compareMode.noTimelineData")}
      </p>
    );
  ...
  const poly = (data: LossEntry[]) =>
    data
      .filter((_, i) => i % Math.ceil(data.length / 150) === 0 || i === data.length - 1)
      .map((e) => `${toX(e.step).toFixed(1)},${toY(e.value).toFixed(1)}`)
      .join(" ");
    ...
    <div className="space-y-1">
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ maxHeight: 140 }}>
        ...
        {valLoss.length >= 2 && (
          <text
            x={w - padR}
            y={toY(valLoss[valLoss.length - 1].value) - 3}
            fontSize={8}
            fill="rgb(52,211,153)"
        ...
        <text
          ...
          fontFamily="monospace"
        >
          {t("models.mlp.compareMode.steps")}
        </text>
        <text
          x={padL - 2}
          y={toY(maxVal) + 3}
      ...
      <div className="flex gap-4 text-[8px] font-mono text-white/20">
        <span>
          <span className="inline-block w-2 h-0.5 bg-violet-500/50 mr-1 align-middle" />
          {t("models.mlp.compareMode.train")}
        </span>
        <span>
          <span className="inline-block w-2 h-0.5 bg-emerald-400 mr-1 align-middle" />
          {t("models.mlp.compareMode.val")}
        </span>
      </div>
    </div>
  );
...
function ConfigPanel({ config, label, seed }: ConfigPanelProps) {
  ...
  const trainLoss = useMemo(
    () =>
      (timeline?.metrics_log?.train_loss ?? []).map((e) => ({
        step: e.step,
  ...
  const valLoss = useMemo(
    () =>
      (timeline?.metrics_log?.val_loss ?? []).map((e) => ({
        step: e.step,
        value: typeof e.value === "number" ? e.value : 0,
      })),
    [timeline],
  ...
  return (
    <div className="space-y-4 min-w-0">
      {/* Config badge */}
      <div
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border text-[10px] font-mono ${badgeClass}`}
      >
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          {
            label: t("models.mlp.compareMode.metrics.valLoss"),
            value: config.final_loss.toFixed(3),
          },
          {
            label: t("models.mlp.compareMode.metrics.perplexity"),
            value: config.perplexity.toFixed(1),
          },
          {
            label: t("models.mlp.compareMode.metrics.gap"),
            value:
              config.generalization_gap != null
                ? (config.generalization_gap > 0 ? "+" : "") + config.generalization_gap.toFixed(3)
                : "—",
          },
          {
            label: t("models.mlp.compareMode.metrics.score"),
            value: config.score != null ? config.score.toFixed(2) : "—",
          },
        ].map(({ label: l, value }) => (
          <div
            key={l}
          ...
          >
            <p className="text-[8px] font-mono uppercase tracking-widest text-white/20 mb-0.5">
              {l}
            </p>
      ...
      <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
        {tlLoading ? (
          ...
          <SimpleChart
            trainLoss={trainLoss}
            valLoss={valLoss}
            label={t("models.mlp.compareMode.trainingLoss")}
          />
        )}
      </div>
      ...
      <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
        <p className="text-[9px] font-mono uppercase tracking-widest text-white/20 mb-2">
          {t("models.mlp.compareMode.embeddingSpace")}
        </p>
      ...
      <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3 space-y-2">
        <p className="text-[9px] font-mono uppercase tracking-widest text-white/20">
          {t("models.mlp.compareMode.generatedText")}
        </p>
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-mono text-white/25 shrink-0">
            {t("models.mlp.compareMode.temperature")}={temperature.toFixed(1)}
          </span>
          <input
            type="range"
            min={1}
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded text-[9px] font-mono border transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
              label === "A"
          ...
          >
            {genLoading ? (
              ...
              t("models.mlp.compareMode.generate")
            )}
          </button>
        </div>
        {generation ? (
          ...
          <p className="text-[9px] text-white/15 italic font-mono">
            {t("models.mlp.compareMode.seedTextAboveWillBeUsed")}
          </p>
        )}
      </div>
    </div>
...
function ConfigSelector({ configs, value, exclude, onChange, label }: SelectorProps) {
  ...
  const accentClass = label === "A" ? "focus:border-violet-500/40" : "focus:border-cyan-500/40";

  return (
    <div className="space-y-1">
      <p className="text-[9px] font-mono uppercase tracking-widest text-white/25">
        {t("models.mlp.compareMode.config")} {label}
      </p>
      <select
      ...
      >
        <option value="" disabled>
          {t("models.mlp.compareMode.selectAConfig")}
        </option>
        {options.map((c) => (
          <option key={c.config_id} value={c.config_id}>
            emb={c.embedding_dim} · h={c.hidden_size} · lr={c.learning_rate} · loss=
            {c.final_loss.toFixed(3)}
          </option>
        ))}
      </select>
    </div>
  ...
  if (configs.length < 2) {
    return (
      <p className="text-[10px] text-white/20 font-mono italic">
        {t("models.mlp.compareMode.needAtLeastTwoConfigs")}
      </p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-[11px] text-white/30 font-mono leading-relaxed">
        {t("models.mlp.compareMode.selectTwoConfigsToCompare")}
      </p>
          ...
          {configA ? (
            ...
            <div className="flex items-center justify-center py-12 rounded-lg border border-dashed border-white/[0.06]">
              <p className="text-[10px] text-white/20 font-mono">
                {t("models.mlp.compareMode.selectConfigA")}
              </p>
            </div>
          )}
          {configB ? (
            ...
            <div className="flex items-center justify-center py-12 rounded-lg border border-dashed border-white/[0.06]">
              <p className="text-[10px] text-white/20 font-mono">
                {t("models.mlp.compareMode.selectConfigB")}
              </p>
            </div>
          )}
        </div>
        ...
        (() => {
          ...
          return (
            <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4 space-y-2">
              <p className="text-[9px] font-mono uppercase tracking-widest text-white/20">
                {t("models.mlp.compareMode.diffSummary")}
              </p>
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  {
                    label: t("models.mlp.compareMode.metrics.params"),
                    diff: paramDiff,
                    unit: "",
                    better: false,
                    neutral: true,
                  ...
                  <div key={label} className="text-center">
                    ...
                    <p
                      className={`text-sm font-mono font-bold ${neutral ? "text-white/40" : better ? "text-emerald-400" : "text-rose-400"}`}
                    >
                      {diff > 0 ? "+" : ""}
                      {unit}
                      {Math.abs(diff) >= 1000 ? `${(diff / 1000).toFixed(1)}k` : diff.toFixed(3)}
                    </p>
                  </div>
                ))}
              </div>
