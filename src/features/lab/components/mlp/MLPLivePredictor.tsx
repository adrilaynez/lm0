...
export function MLPLivePredictor() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      setBackendAvailable(false);
      const key =
        Object.keys(FALLBACK_SAMPLES).find((k) => seed.startsWith(k)) ??
        Object.keys(FALLBACK_SAMPLES)[0];
  ...
  const tempLabel =
    temperature <= 0.5 ? "Conservative" : temperature <= 1.0 ? "Balanced" : "Creative";

  return (
    <div className="p-4 sm:p-5 space-y-4">
      {/* Seed input */}
      <div className="space-y-2">
        <label className="text-[9px] font-mono text-white/25 uppercase tracking-widest">
          Seed text
        </label>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 space-y-0.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono text-violet-400/60 tabular-nums">
