...
const ACTIVATION_NOTES: Record<string, string> = {
  ...
  sigmoid:
    "S-curve, output in [0, 1]. Double saturation at both ends. Worse gradient flow than tanh.",
  linear:
    "f(x) = x — no non-linearity at all. Stacking linear layers = one big linear function. Cannot learn.",
};
  ...
  models: [
    {
      label: "act_relu",
      activation: "relu",
      final_val_loss: 1.658,
      final_train_loss: 1.51,
      diverged: false,
      loss_curve: {
        train: [3.3, 2.8, 2.3, 1.9, 1.7, 1.52],
        val: [3.3, 2.85, 2.35, 2.0, 1.8, 1.658],
      },
      generated_samples: ["the throne of the kingdom shall not"],
    },
    {
      label: "act_gelu",
      activation: "gelu",
      final_val_loss: 1.695,
      final_train_loss: 1.55,
      diverged: false,
      loss_curve: {
        train: [3.3, 2.82, 2.32, 1.92, 1.72, 1.56],
        val: [3.3, 2.87, 2.37, 2.02, 1.82, 1.695],
      },
      generated_samples: ["the throne of the king henry"],
    },
    {
      label: "act_tanh",
      activation: "tanh",
      final_val_loss: 1.703,
      final_train_loss: 1.58,
      diverged: false,
      loss_curve: {
        train: [3.3, 2.9, 2.4, 2.05, 1.85, 1.59],
        val: [3.3, 2.95, 2.45, 2.1, 1.92, 1.703],
      },
      generated_samples: ["the throne of the k"],
    },
    {
      label: "act_sigmoid",
      activation: "sigmoid",
      final_val_loss: 2.874,
      final_train_loss: 2.75,
      diverged: false,
      loss_curve: {
        train: [3.3, 3.1, 2.95, 2.85, 2.78, 2.76],
        val: [3.3, 3.12, 2.98, 2.88, 2.84, 2.874],
      },
      generated_samples: ["the the the the"],
    },
    {
      label: "act_linear",
      activation: "linear",
      final_val_loss: 2.215,
      final_train_loss: 2.1,
      diverged: false,
      loss_curve: {
        train: [3.3, 3.1, 2.8, 2.5, 2.3, 2.1],
        val: [3.3, 3.12, 2.85, 2.55, 2.35, 2.215],
      },
      generated_samples: ["e e e the and"],
    },
  ],
};
...
export function ActivationBattleVisualizer() {
  ...
  const [enabled, setEnabled] = useState<Set<string>>(
    new Set(["relu", "gelu", "tanh", "sigmoid", "linear"]),
  );
  const [hoveredAct, setHoveredAct] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch(`${process.env.NEXT_PUBLIC_LM_LAB_API_URL ?? ""}/api/v1/mlp/activation-battle`)
      .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
      .then((d: BattleResponse) => {
        if (!cancelled) {
          setData(d);
          setLoading(false);
        }
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence mode="wait">
        {hoveredAct && (
          <motion.div
            ...
            style={{
              borderColor: ACTIVATION_COLORS[hoveredAct] + "30",
              backgroundColor: ACTIVATION_COLORS[hoveredAct] + "08",
            }}
          >
            <p
              className="text-[10px] font-mono leading-relaxed"
              style={{ color: ACTIVATION_COLORS[hoveredAct] + "cc" }}
            >
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-3">
        <div className="text-[9px] font-mono text-white/25 mb-2">
          Validation Loss over Training (lower = better)
        </div>
      ...
      <div className="space-y-1.5">
        {ranked.map((m, rank) => {
          ...
          const lossBarW =
            valLoss && bestLoss ? Math.min(100, ((valLoss - 1.4) / (3.0 - 1.4)) * 100) : 100;
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span
                  ...
                  style={{ color: m.diverged ? "#ef4444" : color }}
                >
