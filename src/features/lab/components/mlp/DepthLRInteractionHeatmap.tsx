...
export function DepthLRInteractionHeatmap() {
  ...
  useEffect(() => {
    let cancelled = false;
    const base = process.env.NEXT_PUBLIC_LM_LAB_API_URL ?? "";
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[var(--lab-viz-bg)] rounded-xl border border-[var(--lab-border)] p-4 overflow-x-auto">
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="text-left text-[var(--lab-text-muted)] pb-2 pr-3 font-normal w-16">
                Depth ↓ · LR →
              </th>
              {LR_LABELS.map((lr) => (
                <th
                  key={lr}
                  className="text-center pb-2 font-mono text-[var(--lab-text-muted)] font-normal px-1"
                >
                  {lr}
                </th>
              ))}
        ...
        {hoveredCell && (
          ...
          >
            <span className="text-[var(--lab-text-muted)]">
              L={hoveredDepth}, LR={hoveredLR}:{" "}
            </span>
            {hovered === null ? (
              <span className="text-red-400 font-semibold">
                DIVERGED — loss exploded during training
              </span>
            ) : (
              <>
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        {[
          {
            label: "High LR + Deep",
            desc: "Gradient explosion — diverges",
            color: "text-red-400",
            bg: "bg-red-500/10 border-red-500/20",
          },
          {
            label: "Low LR + Any",
            desc: "Stable but slow to converge",
            color: "text-amber-300",
            bg: "bg-amber-500/10 border-amber-500/20",
          },
          {
            label: "Med LR + 2–4 layers",
            desc: "Sweet spot — lowest val loss",
