...
/**
 * Verdict — the plain-language sage verdict panel for the Bigram chapter (v8, editorial-green).
 *
 ...
 *
 * Reads only --bigram-* tokens + the registered fonts. Reduced-motion safe.
 * Mirrors styles-v8.css `.bw-corpus__verdict` (.vk / .vmain / .vsub).
 */
...
export const Verdict = memo(function Verdict({ label, main, sub }: VerdictProps) {
    ...
    return (
        ...
        >
            ...
            <span
                className="bigram-verdict__main"
                style={{
            ...
            >
                ...
                <style>{".bigram-verdict__main b{color:var(--bigram-accent-ink);font-weight:600}"}</style>
                {main}
            </span>
            <span
                style={{
