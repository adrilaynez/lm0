/**
 * bigram/kit · tokens — the single place the chapter's fonts, easing, heat ramp and glyph helper live.
 *
 * Every kit primitive and every visualizer imports from here instead of re-declaring `"var(--bigram-...)"`
 * strings inline. Tokens-only: never hardcode a hex; the values below are CSS custom properties resolved
 * under the chapter's `[data-bigram-theme]` scope (defined in globals.css), so they adapt to light/dark.
 */

/** Registered font families (resolve under [data-bigram-theme]). */
export const MONO = "var(--bigram-font-mono)";
export const SERIF = "var(--bigram-font-serif)";
export const DISPLAY = "var(--bigram-font-display)";
