...
/**
 * bigram/kit · MarkedText — the chapter's signature "marking" idiom, in ONE place.
 *
 ...
 * reduced-motion safe. Self-contained `<style>` scoped by `.bw-mt`.
 */
...
export const MarkedText = memo(function MarkedText({
    ...
    const flush = (k: string) => {
        if (word.length) {
            items.push(
                <span key={`w${k}`} className="bw-mt__word">
                    {word}
                </span>,
            );
            word = [];
    ...
    text.split("").forEach((ch, i) => {
        const span = (
            <span key={i} className="bw-mt__ch" data-state={get(i)}>
                {ch === " " ? " " : ch}
            </span>
        );
        if (!wordGroup) {
        ...
        if (ch === " " || ch === "\n") {
            flush(`${i}`);
            items.push(
                <span key={i} className="bw-mt__ch bw-mt__space" data-state={get(i)}>
                    {" "}
                </span>,
            );
        } else {
    ...
    return (
        <p
            className={`bw-mt${className ? ` ${className}` : ""}`}
        ...
        >
            ...
            <style>{`
                ...
                .bw-mt__word { display: inline-flex; align-items: baseline; white-space: nowrap; }
                .bw-mt__ch {
                ...
                .bw-mt__ch[data-state="idle"] { color: var(--bigram-dim); }
                .bw-mt__ch[data-state="past"] { color: color-mix(in oklab, var(--bigram-ink) 38%, var(--bigram-dim)); }
                .bw-mt__ch[data-state="future"] { color: color-mix(in oklab, var(--bigram-dim) 52%, transparent); }
                .bw-mt__ch[data-state="cur"] { color: var(--bigram-ink-2); background: color-mix(in oklab, var(--bigram-ink) 6%, transparent); }
                .bw-mt__ch[data-state="hot1"] { color: var(--bigram-on-accent); background: var(--bigram-accent); font-weight: 700; }
                .bw-mt__ch[data-state="hot2"] {
