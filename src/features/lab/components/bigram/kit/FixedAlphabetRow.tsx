...
export interface FixedAlphabetRowProps {
  ...
  /** Makes slots clickable (e.g. "you are the dice" pick-a-letter modes). Optional. */
...
export const FixedAlphabetRow = memo(function FixedAlphabetRow({
  ...
  return (
    <div className="bw-far" data-clickable={clickable ? "1" : "0"}>
      ...
      <div className="bw-far__axis" aria-hidden>
        {cols.map((c, i) => (
          <span
            key={c + i}
            className="bw-far__lbl"
            data-hi={i === (hoverIdx >= 0 ? hoverIdx : winner) ? "1" : "0"}
          >
            {displayChar(c)}
          </span>
        ))}
      ...
      <style>{`
                ...
                .bw-far[data-clickable="1"] .bw-far__bar, .bw-far[data-clickable="1"] .bw-far__cell { cursor: pointer; }
