...
export const AmnesiaReplay = memo(function AmnesiaReplay({ accent }: { accent?: "ngram" }) {
  ...
  return (
    <div style={{ width: "100%", maxWidth: 640, margin: "0 auto", padding: "6px 0 10px" }}>
      <CaptionLine align="center">
        {blind ? "lo único que recuerda" : "tres palabras distintas"}
      </CaptionLine>
        ...
        {interacted && (
          ...
          >
            <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
              <span
                aria-hidden
                style={{ width: 14, height: 1, background: "var(--ngram-accent)", opacity: 0.5 }}
              />
              <span
                style={{
                  fontFamily: MONO,
      ...
      >
        <PlayButton
          onClick={next}
          aria-label="ver otras tres palabras que colapsan en la misma apuesta"
        >
          otra vez
        </PlayButton>
        <GhostButton
...
function Funnel({ reduce }: { reduce: boolean }) {
  return (
    <svg
      ...
      style={{ width: "100%", height: 56, marginTop: 6, overflow: "visible" }}
    >
      {[50, 150, 250].map((x, i) => (
        <motion.path
          key={i}
