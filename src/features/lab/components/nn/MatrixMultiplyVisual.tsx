...
function matMul(x: number[], w: number[][]): number[] {
  return w[0].map((_, col) => x.reduce((sum, xi, row) => sum + xi * w[row][col], 0));
}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap mb-3">
          ...
          <div className="flex flex-col gap-1 items-center">
            ...
            {X_VEC.map((val, i) => (
              <div
                ...
                style={{
                  borderColor: NN_COLORS.input.hex + "30",
                  background: NN_COLORS.input.hex + "10",
                  color: NN_COLORS.input.hex,
                }}
              >
                x{i + 1}={val}
              </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <div
              className="grid grid-cols-2 rounded-lg border border-white/[0.08] overflow-hidden"
              style={{ gap: 1 }}
            >
              {W.flatMap((row, r) =>
                row.map((val, c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{ color: isHi ? NN_COLORS.weight.hex : "rgba(255,255,255,0.5)" }}
                      >
                        {val}
                      </span>
                    </div>
          ...
          <div className="flex flex-col gap-1 items-center">
            ...
            {Y_VEC.map((val, i) => (
              <div
                ...
                style={{
                  borderColor:
                    highlightCol === i ? NN_COLORS.output.hex + "60" : NN_COLORS.output.hex + "30",
                  background:
                    highlightCol === i ? NN_COLORS.output.hex + "15" : NN_COLORS.output.hex + "08",
        ...
        {highlightCol !== null && (
          ...
          >
            y{highlightCol + 1} = {X_VEC.map((x, i) => `${x} × ${W[i][highlightCol]}`).join(" + ")}{" "}
            = {Y_VEC[highlightCol].toFixed(1)}
          </motion.div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
        ...
        <div className="space-y-2 text-[11px] font-mono text-white/40">
          <p>
            • input: <span style={{ color: NN_COLORS.input.hex }}>(1 × n)</span> → n features
          </p>
          <p>
            • weights: <span style={{ color: NN_COLORS.weight.hex }}>(n × m)</span> → n inputs to m
            outputs
          </p>
          <p>
            • output: <span style={{ color: NN_COLORS.output.hex }}>(1 × m)</span> → m features
          </p>
        </div>
      ...
      <div>
        <button
          ...
          className={`w-full px-4 py-2 rounded-lg text-[11px] font-mono font-bold border transition-all ${
            showStacking
