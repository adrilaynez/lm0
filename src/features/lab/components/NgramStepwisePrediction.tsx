...
/**
 * NgramStepwisePrediction — the N-gram chapter's stepwise tracer.
 *
 * This is the pre-v8 (cyan) implementation, kept intact for the N-gram chapter, which owns its own
 * accent and must NOT be recolored to the Bigram editorial-green. The Bigram chapter uses the
 * redesigned `StepwisePrediction` (editorial-green, HonestBar + Verdict); the two are intentionally
 * separate so neither chapter regresses the other.
 */

interface NgramStepwisePredictionProps {
...
export function NgramStepwisePrediction({
    onPredict,
    steps,
    finalPrediction,
    loading,
    error,
}: NgramStepwisePredictionProps) {
    const { t } = useI18n();
