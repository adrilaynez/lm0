    ...
    return (
        <div className="p-5 sm:p-6 space-y-4">
            ...
            <div className="flex justify-center">
                <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-md">
                    ...
                    {SCHEDULES.map((s, i) => (
                        <polyline
                            key={s.id}
                            points={s.lossCurve.map((v, j) => `${toX(j)},${toLossY(v)}`).join(" ")}
                            fill="none"
                            stroke={s.color}
                    ...
                    {/* LR curve for selected */}
                    <polyline
                        points={schedule.lrCurve.map((v, i) => `${toX(i)},${toLrY(v)}`).join(" ")}
                        fill="none"
                    ...
                    />
                    {/* Y-axis labels */}
                    <text x={pad - 3} y={toLossY(2.0) + 3} textAnchor="end" fill="rgba(255,255,255,0.12)" fontSize="7" fontFamily="monospace">2.0</text>
                    <text x={pad - 3} y={toLossY(3.0) + 3} textAnchor="end" fill="rgba(255,255,255,0.12)" fontSize="7" fontFamily="monospace">3.0</text>
                </svg>
            </div>
            ...
            <div className="flex gap-4 justify-center text-[9px] font-mono">
                ...
                <span className="flex items-center gap-1"><span className="w-4 h-0.5 rounded" style={{ backgroundColor: schedule.color, opacity: 0.5, borderTop: `1px dashed ${schedule.color}` }} /> LR</span>
            </div>
