export const en = {
        ...
        mlp: {
            ...
            narrative: {
                hero: {
                    ...
                    titlePrefix: "The Input Problem:",
                    titleHighlight: "MLP + Embeddings",
                    description: "Our neural network can learn — but it only sees one letter at a time. How do we feed context into a network, and what happens when we let it learn its own representations?",
                    readTime: "~45 min read · 50+ interactive demos",
                },
                sections: {
                    s01: { number: "01", label: "The Input Problem" },
                    s02: { number: "02", label: "Representation" },
                    s03: { number: "03", label: "Embeddings in Action" },
                    s04: { number: "04", label: "The Full Pipeline" },
                    s05: { number: "05", label: "Going Deeper" },
                    s06: { number: "06", label: "Stability" },
                    s07: { number: "07", label: "Hyperparameters" },
                    s08: { number: "08", label: "Limitations" },
                    s09: { number: "09", label: "Looking Forward" },
                },
                s01: {
                    heading: "The Input Problem: Feeding Context to a Network",
                    lead: "Our neural network can learn — but it's blind. It sees one letter, guesses the next, and forgets. To build something smarter, we need to solve a fundamental problem: how do you feed CONTEXT into a neural network?",
                    evolutionLabel: "The journey so far",
                    bigramLabel: "Bigram",
                    bigramDesc: "1 letter, counting",
                    ngramLabel: "N-gram",
                    ngramDesc: "N letters, counting (explodes!)",
                    nnLabel: "Neural Net",
                    nnDesc: "1 letter, learning",
                    mlpLabel: "MLP",
                    mlpDesc: "N letters, learning ← this!",
                    p1: "In the Neural Networks chapter, we encoded each character as an integer — 'a'=0, 'b'=1, etc. — and fed it to a single neuron. That worked for one-letter context. But what if we want to see 3 letters at once?",
                    p2: "The simplest idea: take 3 characters, convert each to a",
                    p2H1: "one-hot vector",
                    p2End: " (a list of zeros with a single 1), then concatenate them into one long input vector.",
                    ...
                    p3: "This works! But there's a catch. Each one-hot vector has V dimensions (one per character in the vocabulary). With context window N, the input vector has",
                    p3H1: "N × V",
                    p3End: " dimensions. As the context grows, the input — and the weight matrix connecting it to the hidden layer — grows with it.",
                    figLabel2: "Interactive · Dimensionality Scale",
                    figHint2: "Drag the slider to increase context size. Watch the input dimensions and weight matrix explode — then compare with the embedding approach.",
                    p4: "With a vocabulary of 27 characters and context of 3, the input is 81 dimensions — fine. But with context of 10, it's 270. With 20, it's 540. And the first weight matrix W₁ grows proportionally.",
                    formulaCaption: "The input to the MLP is a concatenation of N one-hot vectors, one per context token.",
                    calloutTitle: "From one letter to many",
                    calloutText: "This is the key step: instead of feeding one letter, we feed N letters. The N-gram had this idea but stored counts in exponentially growing tables. The MLP learns a compact function — same context, dramatically fewer parameters.",
                    archLabel: "MLP Architecture · Schematic",
                    archHint: "The architecture you just designed: N one-hot context vectors → concatenated input → hidden layers → softmax output.",
                    p5: "Does context actually help? Let's find out. Below, type some text and compare predictions from a model that sees 1 letter versus one that sees 3.",
                    figLabel3: "Interactive · Context vs No Context",
                    figHint3: "Type text and compare predictions: context=1 (only the last letter) versus context=3 (last 3 letters). See how context improves accuracy.",
                    takeaway: "Feeding multiple characters as concatenated one-hot vectors gives the MLP context — the ability to see patterns across characters. But the input size grows linearly with context, and every character is equally distant from every other. Can we do better?",
                },
                s02: {
                    heading: "The Representation Problem: Beyond One-Hot",
                    lead: "Every character is the same distance from every other character. The network gets zero information about similarity from the encoding. What if we could give characters meaningful coordinates?",
                    ...
                    p2: "By sorting characters into groups, you just did something remarkable: you assigned each letter a set of features. What if instead of categories, you assigned",
                    p2H1: "numbers",
                    p2End: "? Then each letter would become a point in a coordinate space, and similar letters would naturally end up close together.",
                    ...
                    p3: "With meaningful features, vowels naturally cluster together and rare consonants group apart. But who chooses the features? We could hand-craft them... or we could let the network",
                    p3H1: "learn them automatically",
                    p3End: ". This is the key insight: define a matrix E of shape V × D, where V is the vocabulary size and D is the embedding dimension. Each row is a character's learned representation.",
                    ...
                    takeaway: "Embeddings replace wasteful one-hot vectors with dense, learned representations where similar tokens get similar vectors. Instead of N × V input dimensions, we now have N × D — orders of magnitude smaller, with similarity built in.",
                },
                s03: {
                    heading: "Embeddings in Action: What Do They Learn?",
                    lead: "We told the network to learn its own representations. But what does it actually discover? Let's watch embeddings evolve during training and explore the structure that emerges.",
                    p1: "At step 0, every character's embedding is random noise — no structure at all. As training progresses, the network gradually moves similar characters closer together. Watch this happen in real time below.",
                    ...
                    p2: "By the end of training, vowels cluster together, common consonants form their own group, and rare characters drift to the periphery. The network discovered these groupings on its own — we never told it about vowels or consonants.",
                    figLabel2: "Interactive · Embedding Space Explorer",
                    figHint2: "Click on any character to highlight it and explore the learned embedding space. This is a pedagogical illustration of the clustering pattern.",
                    p3: "How close are two characters in embedding space? We can measure this with",
                    p3H1: "Euclidean distance",
                    p3Mid: " (straight-line distance) and",
                    p3H2: "cosine similarity",
                    p3End: " (angle between vectors). Characters that appear in similar contexts end up with similar embeddings.",
                    ...
                    p4: "For any character, we can find its nearest neighbors — the characters whose embeddings are most similar. This reveals what the network considers 'related'.",
                    figLabel4: "Interactive · Nearest Neighbor Explorer",
                    figHint4: "Select a character and see which other characters have the most similar learned embeddings.",
                    p5: "We can even do arithmetic on embeddings. Group centroids reveal how the network organizes characters internally — and vector operations show whether relationships are encoded as consistent directions.",
                    figLabel5: "Interactive · Embedding Arithmetic",
                    figHint5: "Explore group centroids (vowels vs consonants) and try vector arithmetic: character A − character B + character C ≈ ?",
                    p6: "Does embedding dimension matter? Below, compare embeddings trained with 2, 10, and 32 dimensions. More dimensions capture finer structure — but even 2 dimensions show remarkable clustering.",
                    ...
                    calloutText: "This approach — learning dense representations of discrete tokens and using them as neural network inputs — was proposed by Yoshua Bengio and colleagues in their landmark 2003 paper 'A Neural Probabilistic Language Model.' It laid the foundation for all modern language models.",
                    takeaway: "Trained embeddings are not random — they discover meaningful structure. Vowels cluster, consonants group by type, and the representations improve with more dimensions. This learned structure is what gives the MLP its power to generalize.",
                },
                s04: {
                    heading: "Building the Full Pipeline",
                    lead: "We have context windows, embeddings, and hidden layers. Now let's see how all the pieces connect — from raw text to a prediction — and understand what makes this fundamentally different from counting.",
                    p1: "Let's trace a concrete example. Given the input \"hel\", our MLP needs to predict the next character. Walk through each step below — from characters to token IDs, through the embedding lookup, concatenation, hidden layer, and finally the softmax output.",
                    figLabel1: "Interactive · Forward Pass Step-by-Step",
                    figHint1: "Click 'Next Step' to walk through the MLP's forward pass: input → tokenize → embed → concatenate → hidden layer → softmax prediction.",
                    p2: "That was one forward pass — one prediction. But how does the network actually learn? Training means repeating a cycle: predict, measure the error, compute gradients, and update weights. Watch one full training iteration below.",
                    figLabel2: "Interactive · Single Example Trainer",
                    figHint2: "Step through the training loop: forward pass → cross-entropy loss → backpropagation → weight update. Click 'Next Iteration' to see the loss decrease.",
                    p3: "The N-gram model stored a giant lookup table — one entry for every possible character sequence. The MLP stores compact weight matrices. How do the parameter counts compare?",
                    p3H1: "The difference is dramatic",
                    p3End: " — and it grows exponentially with context size.",
                    figLabel3: "Interactive · N-gram vs MLP Parameters",
                    figHint3: "Adjust vocabulary size, context length, embedding dimension, and hidden size. Compare the N-gram's exponential table with the MLP's compact matrices.",
                    p4: "Now let's use the trained model for real. Type any text below and watch the MLP predict the next character in real time — using the actual trained model, not a simulation.",
                    figLabel4: "Interactive · Live MLP Predictor",
                    figHint4: "Type text and see real-time next-character predictions from a trained MLP. Click predicted characters to append them and keep generating.",
                    p5: "Finally, here's the complete pipeline from start to finish — every component working together as a unified system.",
                    figLabel5: "Interactive · Complete MLP Pipeline",
                    figHint5: "The full MLP language model pipeline: embedding lookup → concatenation → hidden layers → softmax → next character prediction.",
                    calloutTitle: "From counting to learning",
                    calloutText: "The N-gram counted exact sequences and looked them up in a table. The MLP learns compressed representations and computes predictions through matrix operations. This means it can generalize to sequences it has never seen before — as long as the component characters have similar embeddings to ones it has seen.",
                    takeaway: "The MLP pipeline transforms raw characters into predictions through a chain of learned transformations: tokenize → embed → concatenate → hidden layer → softmax. Unlike the N-gram's exponential table, the MLP's parameters grow linearly with context size.",
                },
                s05: {
                    heading: "Going Deeper: What Happens With More Layers?",
                    lead: "One hidden layer works. Language is compositional — letters form patterns, patterns form words, words form phrases. What if we add more layers to capture this hierarchy?",
                    p1: "Let's experiment. Below, add 1 to 4 hidden layers and observe how the model changes — more parameters, potentially lower loss, but also new problems.",
                    figLabel1: "Interactive · Depth Explorer",
                    figHint1: "Select 1-4 hidden layers. See how parameters grow, loss changes, and generation quality is affected. Notice the sweet spot — and the instability beyond it.",
                    p2: "What's happening inside each layer? As data flows through a deep network, the activations at each layer tell us what features it's detecting. Let's look at the activation patterns.",
                    figLabel2: "Interactive · Layer Activation Explorer",
                    figHint2: "Click through layers to see activation heatmaps. In deeper layers, notice how values concentrate at the extremes — this is saturation.",
                    p3: "The activation histogram makes the problem clearer. In layer 1, values are spread healthily. By layer 3, most values are pushed to ±1 — the saturation zone of tanh where",
                    p3H1: "gradients are nearly zero",
                    p3End: ".",
                    figLabel3: "Interactive · Activation Histograms by Layer",
                    figHint3: "Compare activation distributions across layers. Deeper layers show bimodal distributions piling up at ±1 — the tanh dead zone.",
                    p4: "How does depth affect the actual generated text? Compare outputs from models with 1, 2, and 3 layers trained on the same data.",
                    figLabel4: "Interactive · Depth Generation Gallery",
                    figHint4: "Side-by-side text samples from models with different depths. The 2-layer model is the sweet spot — 3 layers produces worse text due to training instability.",
                    calloutTitle: "The depth paradox",
                    calloutText: "More layers should mean more expressive power — and mathematically they do. But in practice, deep networks are much harder to train. The activations saturate, the gradients vanish, and the network ends up worse than a shallower one. Solving this requires understanding WHY training breaks.",
                    takeaway: "Adding layers increases the network's theoretical capacity, but beyond 2 layers, training becomes unstable. Activations saturate at ±1, gradients shrink to near zero, and the model actually gets worse. Understanding this failure is the key to going deeper.",
                },
                s06: {
                    heading: "Training Stability: When Depth Fights Back",
                    lead: "We saw that deep networks should be more powerful but often train worse. Why? The answer involves three interconnected problems — and three solutions that revolutionized deep learning.",
                    p1: "The first problem is initialization. If weights start too large, activations explode. Too small, and the signal fades to nothing before reaching the output. Below, see how three different initialization scales produce wildly different training outcomes.",
                    ...
                    p2: "Why does initialization matter so much? Because of tanh saturation. When inputs to tanh are large, the output is stuck near ±1 and the derivative drops to nearly zero. Drag the slider below to find the dead zone.",
                    figLabel2: "Interactive · Tanh Saturation",
                    figHint2: "Drag x to see tanh(x) and its derivative. When |x| > 2, the derivative approaches 0 — gradients through this neuron are killed.",
                    p3: "The second problem is vanishing and exploding gradients. During backpropagation, gradients are multiplied through each layer. If each factor is less than 1, gradients shrink exponentially. Greater than 1, they explode.",
                    figLabel3: "Interactive · Gradient Product Simulator",
                    figHint3: "Set a per-layer gradient factor and number of layers. Watch how the total gradient vanishes or explodes — even small deviations from 1.0 compound dramatically.",
                    p4: "The visualization below shows gradient magnitudes flowing backward through a network. In the vanishing regime, early layers receive almost no learning signal.",
                    figLabel4: "Interactive · Gradient Flow",
                    ...
                    p5: "Solution 1: Better initialization. Kaiming initialization scales weights based on layer size, keeping the variance stable through each layer. Compare it against naive random initialization.",
                    figLabel5: "Interactive · Initialization Comparison",
                    figHint5: "Side-by-side training: random init vs Kaiming init. Watch how Kaiming converges faster and reaches lower loss.",
                    p6: "Solution 2: Batch Normalization. Without it, activation distributions drift unpredictably during training — the 'internal covariate shift' problem. Batch norm re-centers and re-scales activations at each layer.",
                    figLabel6: "Interactive · Activation Drift",
                    figHint6: "Toggle between 'Without Batch Norm' and 'With Batch Norm'. Watch how distributions drift vs stay stable across training steps.",
                    p7: "Here's the batch normalization formula in action — normalizing activations to zero mean and unit variance, then applying learned scale and shift parameters.",
                    figLabel7: "Interactive · Batch Norm Effect",
                    figHint7: "Toggle BatchNorm on and off to see how it stabilizes activation distributions across layers.",
                    p8: "Solution 3 (preview): Residual connections. Instead of y = f(x), compute y = x + f(x). The original signal always passes through, giving gradients a direct highway back to early layers.",
                    figLabel8: "Interactive · Residual Connections",
                    figHint8: "Compare y = f(x) vs y = x + f(x). See how the skip connection preserves signal strength through deep layers.",
                    calloutTitle: "The deep learning toolkit",
                    calloutText: "Kaiming initialization (2015), Batch Normalization (Ioffe & Szegedy, 2015), and Residual Connections (He et al., 2015) — all published in the same year — transformed deep learning from unreliable to routine. These three techniques together enabled training networks with 100+ layers.",
                    takeaway: "Deep training fails because of saturation, vanishing gradients, and distribution drift. The solutions — proper initialization, batch normalization, and residual connections — address each problem directly. Together, they make deep networks trainable.",
                },
                s07: {
                    heading: "Exploring the Landscape: Hyperparameters",
                    lead: "We've built the pipeline, explored depth, and tamed training instability. Now the practical question: how big should our MLP be? What settings produce the best model?",
                    ...
                    p3: "The learning rate schedule controls how aggressively the optimizer updates weights over time. A constant rate often oscillates; decay and warmup strategies help.",
                    ...
                    p5: "Now put it all together. The full hyperparameter explorer lets you compare real trained models from our grid — see how all these choices interact.",
                    ...
                    takeaway: "Hyperparameters control the trade-off between capacity and generalization. Embedding dimension, hidden size, learning rate schedule, and temperature each shape model behavior. The best configuration balances expressiveness with the ability to generalize to unseen data.",
                },
                s08: {
                    heading: "The Limits of the MLP",
                    lead: "We've optimized everything we can. The MLP is as good as it gets — and it's still fundamentally limited. Understanding exactly where it breaks reveals what the next architecture must solve.",
                    ...
                    p2H1: "Limitation 1: Fixed context window.",
                    p2: "The MLP must receive exactly N characters. It cannot look further back or dynamically expand its view. Information outside the window — no matter how important — is completely invisible.",
                    ...
                    p3H1: "Limitation 2: Position-dependent meaning.",
                    p3: "Because the MLP concatenates embeddings end to end, the same token at different positions activates completely different weight columns. There is no shared understanding of what a token means — only what it means at this specific position.",
                    ...
                    figLabel4: "Interactive · Position Weight Sharing",
                    figHint4: "See exactly which W₁ columns activate for 'e' at position 0, 1, or 2. No weights are shared across positions.",
                    p4H1: "Limitation 3: Long-range dependencies are unreachable.",
                    p4: "In real language, a pronoun may refer to a noun dozens of tokens earlier. No practically-sized fixed window can reliably bridge these gaps.",
                    ...
                    p5H1: "Limitation 4: Concatenation bottleneck.",
                    ...
                    p6H1: "Limitation 5: No parameter sharing.",
                    p6: "The MLP learns completely separate weights for each position. It can't recognize that 'the' at position 0 and 'the' at position 2 are the same word. This wastes parameters and prevents generalization across positions.",
                    figLabel7: "Interactive · Parameter Sharing Motivation",
                    figHint7: "Compare MLP (separate weights per position) vs a hypothetical shared-weight model. See how much more efficient sharing would be.",
                    p7: "All five limitations share a common origin: the MLP treats its entire context as one flat vector. It has no mechanism to reason about the structure, ordering, or relative importance of individual tokens. What would the ideal architecture look like?",
                    ...
                    takeaway: "The MLP's fixed context window, position-dependent weights, and lack of parameter sharing are hard architectural constraints. Overcoming them requires fundamentally different approaches: processing sequences step by step, sharing weights across positions, and attending selectively to relevant context. These are exactly the ideas behind RNNs and Transformers.",
                },
                s09: {
                    heading: "What We Built: The MLP's Place in History",
                    lead: "Let's step back and see how far we've come — from counting tables to learned representations. The MLP marks a turning point in the history of language modeling.",
                    p1: "Compare every model we've built, side by side. From the simple bigram to the full MLP with embeddings — each step added something essential.",
                    figLabel1: "Interactive · Model Evolution Comparison",
                    figHint1: "Switch between Bigram, N-gram, Neural Net, and MLP+Embeddings. Compare loss, parameters, sample quality, strengths, and weaknesses.",
                    p2: "See the quality difference in generated text. The same seed produces wildly different results depending on the model's architecture and training.",
                    figLabel2: "Interactive · Generation Gallery",
                    figHint2: "Select a seed and compare side-by-side text generation from all four model types. Watch the quality improve with each architectural advance.",
                    p3: "The MLP didn't appear in isolation. It sits at the beginning of a remarkable trajectory of architectural innovation, from sequential memory to attention to the Transformer revolution.",
                    figLabel3: "Interactive · Historical Timeline",
                    figHint3: "From Bengio 2003 to GPT — see where the MLP fits in the story of language modeling.",
                    p4: "The structural limits you explored point naturally toward the next architecture:",
    ...
    bigramNarrative: {
        ...
        coreIdea: {
            ...
            formalTitle: "Deeper: Where Does the Name Come From?",
            formalP1: "The prefix 'bi-' means two, and '-gram' comes from the Greek gramma (letter). A bigram is literally a two-letter unit. In NLP, the term extends to any pair of consecutive tokens — characters, words, or subwords.",
            formulaCaption: "Probability of character cₙ given the previous character cₙ₋₁",
            formalP2: "In plain English: to predict what comes after a character, count how many times that specific pair appeared in the training text, and divide by how many times the first character appeared at all. That ratio is the probability.",
            formalP3: "This is a Maximum Likelihood Estimate (MLE) — the simplest way to turn counts into probabilities. It's the same math behind weather forecasts ('it rained 30 out of 100 Tuesdays, so P(rain|Tuesday) = 30%') and batting averages. Andrey Markov formalized this approach in 1913 when he analyzed letter transitions in Eugene Onegin.",
        ...
        sampling: {
            ...
            softmaxTitle: "Formal: What Is Softmax?",
            softmaxP1: "The technique we just used — turning raw counts into a probability distribution — has a formal name: softmax. It takes any list of numbers (called logits) and squashes them into probabilities that sum to 1, while preserving relative order.",
        ...
        contextBlindness: {
            ...
            explanation: "The bigram only sees the last letter 'h'. The 't', 's', and 'w' before it are completely invisible. That's the fatal flaw.",
            figureLabel: "Context Blindness",
            figureHint: "Three different contexts, three identical predictions. The model is blind to everything except the last character.",
        },
        cliffhanger: {
            ...
            p1: "Ask the model what comes after 'th'. It doesn't know about 't' — it only sees 'h'. So it gives the exact same prediction as 'sh' or 'wh'. The context before 'h' is invisible. Gone forever.",
            memoryLabel: "Interactive · One Letter of Memory",
            memoryHint: "See how the model is stuck with only one character of context.",
            bridgeP1: "The memory limit is only half the story. Watch what happens when three completely different prefixes share the same last letter:",
    ...
    ngramNarrative: {
        ...
        moreContext: {
            ...
            tryPrompt: "Drag the slider. What happens to the prediction?",
        },
        contextWindow: {
        ...
        howItWorks: {
            ...
            bridge: "Each row represents a specific context. Compare 'th' vs 'sh' — this is exactly the distinction the bigram couldn't make. The widget below puts bigram and trigram counting side by side on the same training text so you can see the difference directly.",
        },
        improvement: {
            label: "Improvement",
            ...
            battleBridge: "The numbers tell the story — but seeing is believing. Below, the same seed feeds models with different memory sizes.",
            expandableGenTitle: "Try It Yourself: Your Own N-gram Generator",
        },
        celebration: {
            text: "You've built a much more powerful predictor. The 4-gram writes phrases that almost look like English. But what's the cost of all this memory?",
        },
        whyNotMore: {
            title: "Why Not N=100?",
        ...
        deeperProblem: {
            ...
            demoBridge: "These aren't edge cases — they're everyday situations. Expand the sections below to see how typos and word similarity each break the model in a different way.",
        ...
        statisticalEra: {
            ...
            ngramSummary: "More context, sharper predictions. But the table explodes exponentially, and the model still can't generalize from what it has seen.",
            unknownLabel: "???",
    ...
    neuralNetworkNarrative: {
        ...
        fromNumbers: {
            ...
            mlpBridge: "To fix this, we need two things: more context and a smarter way to represent characters. More context means looking at 2, 3, or more previous characters instead of just one. Smarter representations mean learning what characters have in common — so 'a' and 'e' are recognized as similar vowels, not unrelated symbols. The MLP chapter solves both problems.",
