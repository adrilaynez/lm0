...
const NgramTechnicalExplanation = dynamic(() =>
  import("@/features/lab/components/NgramTechnicalExplanation").then(
    (m) => m.NgramTechnicalExplanation,
  ),
);
...
function LabSection({
  ...
  return (
    <FadeInView margin="-60px">
      <div
        className={`rounded-2xl border ${a.border} bg-gradient-to-br from-white/[0.02] to-black/20 overflow-hidden`}
      >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 px-4 py-3">
                      ...
                      <span
                        className={`text-sm font-bold flex-1 leading-tight ${EXP_TEXT[exp.accent]}`}
                      >
                      ...
                      {isExpanded && (
                        ...
                        >
                          <div className="px-4 pb-4 pt-3 border-t border-white/[0.05] flex flex-col gap-3">
                            ...
                            <div
                              className={`rounded-lg border ${EXP_BORDER[exp.accent]} bg-white/[0.015] px-3 py-2.5`}
                            >
                              ...
                              <p
                                className={`text-xs leading-relaxed ${EXP_TEXT[exp.accent]} opacity-60`}
                              >
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                document
                                  .getElementById(exp.scrollTarget)
            ...
            <div className="pt-4 space-y-4">
              ...
              {hasPerformanceData && (
                ...
                >
                  <NgramPerformanceSummary
                    ...
                    trainingDuration={
                      (training as unknown as { training_duration_ms?: number } | null)
              ...
              {hasLossHistory && displayedLossHistory && (
                <LabSection
                  ...
                  description={t("models.ngram.lab.sections.trainingQualityDesc").replace(
                    "{n}",
                    String(contextSize),
                  )}
                  accent="amber"
...
function NgramPageContent() {
    ...
    visualizeNgram("hello", n, 5)
      .then((res) => {
        ...
        const entry: ComparisonEntry = {
          perplexity:
            res.visualization.training?.perplexity ??
            res.visualization.diagnostics?.perplexity ??
            null,
          contextUtilization:
            res.visualization.training?.context_utilization ??
            res.visualization.diagnostics?.context_utilization ??
            null,
          contextSpace:
            res.visualization.training?.context_space_size ??
            res.visualization.diagnostics?.estimated_context_space ??
            null,
        };
      ...
      .catch(() => {
        if (!active) return;
        comparisonCacheRef.current = {
          ...comparisonCacheRef.current,
          [n]: { perplexity: null, contextUtilization: null, contextSpace: null },
        };
        setComparisonMetrics({ ...comparisonCacheRef.current });
      })
      .finally(() => {
        fetchingNsRef.current.delete(n);
      });
    return () => {
      active = false;
    };
    ...
    () =>
      fallbackCurrent?.probabilities
        ? {
            ...
            col_labels:
              fallbackCurrent.row_labels ??
              Array.from({ length: fallbackCurrent.probabilities.length }, (_, i) => `#${i}`),
          }
        : null,
    [fallbackCurrent],
  );
    ...
    return (
      <LabShell>
        <ErrorBoundary fallbackMessage="The n-gram narrative encountered an error">
          <NgramNarrative contextSize={viz.contextSize} vocabSize={vocabForScalability} />
        </ErrorBoundary>
      </LabShell>
    );
  }
  ...
  const heroStats = diagnostics
    ? [
        ...
        },
        {
          ...
          value:
            training?.total_tokens != null ? `${(training.total_tokens / 1000).toFixed(1)}k` : "?",
          icon: FlaskConical,
  ...
  const hasPerformanceData =
    nGramData?.metadata.inference_time_ms != null || !!nGramData?.metadata.device;
    ...
    <LabShell>
      <div className="max-w-7xl mx-auto pb-24 relative">
        ...
        <div
          ...
          className="max-w-6xl mx-auto px-6 mb-4 grid grid-cols-1 lg:grid-cols-5 gap-6"
        >
