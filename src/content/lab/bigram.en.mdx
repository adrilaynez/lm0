...
{/* FillTheBlank — prediction doesn't require understanding; it only needs the pattern that came before. (Mechanics: see the .tsx) */}
...
{/* HeroAutoComplete — your brain does this already: one letter pulls the next from your mind without thinking. */}
...
{/* PairHighlighter — walking through pairs letter by letter reveals which combinations repeat — that's the hidden pattern. */}
...
{/* IsolateT — the same letter learns different rules depending on its text — small samples teach lies. */}
...
{/* RowTally — reading a whole book reveals real patterns — no one taught them; the count just finds them. */}
...
{/* NormalizationVisualizer — raw counts become bets: divide by the total and the numbers become probabilities that sum to 100%. */}
...
{/* AlwaysMaxLoop — always picking the highest odds locks the machine in a dead loop: the same letter forever. */}
...
{/* LoadedDie — a loaded die that respects the odds: mostly likely, but sometimes surprising — it breaks the loop. */}
...
{/* TinyMatrixExample — new letters create new rows; stacking them makes the matrix — one object holding all the rules. */}
...
{/* GrowingMatrix27 — structure emerges from counting — read a real book and watch the 27×27 table heat itself into being. */}
...
{/* DetectiveMatrix — the full matrix is a language's source code: dark cells are rules the machine discovered just by counting. */}
...
{/* LetterByLetter — one generation step in slow motion, repeated: each output becomes the next input — and the machine writes. */}
...
{/* TableWriter — that step in a loop at full speed: the machine writes on its own, sounding like language from afar, gibberish up close. */}
...
{/* Reveal — the quiet moment of naming: everything you just built has a name that's already existed since 1948. */}
...
{/* Specimen — Shannon built this 75 years ago; his machine wrote exactly like yours: the same babble with a good accent. */}
...
{/* ContextBlindnessDemo — it only sees the last letter — «th», «sh», «wh» are identical to it. No amount of data fixes this ceiling. */}
...
{/* ShannonContextLadder — more context means sharper prediction: blind with one letter, almost certain with most of the word. */}
