...
{/* FillTheBlank — el lector descubre, viéndolo, que predecir no requiere entender: basta mirar lo anterior y adivinar. (Mecánica: ver el .tsx) */}
...
{/* HeroAutoComplete — la máquina hace en vivo lo que tu cabeza hace sin pensarlo: ve una letra y apuesta por la siguiente. */}
...
{/* PairHighlighter — al caminar una frase de dos en dos, los pares repetidos saltan a la vista como el patrón invisible. */}
...
{/* IsolateT — la misma letra en textos distintos elige compañeras distintas: el texto corto miente sobre lo que vendrá. */}
...
{/* RowTally — leer un libro entero y contar de verdad: los patrones reales emergen sin que nadie los enseñe. */}
...
{/* NormalizationVisualizer — los conteos crudos no sirven; dividirlos por el total convierte números en apuestas (suman 100%). */}
...
{/* AlwaysMaxLoop — elegir siempre lo más probable atrapa la máquina en un bucle muerto: la misma letra una y otra vez. */}
...
{/* LoadedDie — un dado trucado que respeta los porcentajes: casi siempre lo probable, pero a veces sorprende y escapa del bucle. */}
...
{/* TinyMatrixExample — cada letra nueva crea una fila de 27 columnas; apilarlas es la matriz, el objeto que resume todas las reglas. */}
...
{/* GrowingMatrix27 — la estructura no se impone; emerge del conteo: leer un libro real y ver la tabla 27×27 calentarse sola. */}
...
{/* DetectiveMatrix — la matriz completa es el código fuente del idioma: casillas oscuras son reglas que nadie escribió, solo contó. */}
...
{/* LetterByLetter — un paso de generación en cámara lenta, repetido: el output de hoy se vuelve el input de mañana, y la máquina escribe. */}
...
{/* TableWriter — ese paso en bucle, sin frenos: la máquina escribe sola, suena a idioma de lejos, galimatías de cerca. */}
...
{/* Reveal — el momento de nombrar en voz baja: todo lo que construiste, desde cero, tiene un nombre que ya existe desde 1948. */}
...
{/* Specimen — Shannon lo hizo hace 75 años; su máquina escribe exactamente como la tuya: el mismo galimatías con buen acento, nada más. */}
...
{/* ContextBlindnessDemo — solo ve la última letra: «th», «sh», «wh» son lo mismo para ella. No se arregla con datos; es el techo del modelo. */}
...
{/* ShannonContextLadder — más contexto, mejor predicción: ciega con una letra, casi segura con la palabra casi completa. El defecto, arreglado. */}
